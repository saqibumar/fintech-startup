...
class StatementTable extends React.Component {
    ...
    componentDidMount(){
        document.getElementById("test-table-xls-button").innerHTML=`<span>
        <svg xmlns="http://www.w3.org/2000/svg" fill="blue" width="24" height="24" viewBox="0 0 24 24"><path d="M17 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V7l-4-4zm-5 16c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm3-10H5V5h10v4z"/></svg> 
        Save xls</span>`;
    ...
    render(){
        ...
        return(
            <div className="xcontainer">
                <div className="row">
                <div className="col-6">
                    <button className="btn-normal pull-left" 
                        onClick={()=>{
                            if(this.props.sys === undefined){
                                window.location.href="/customer/orders";
                            }
                            if(this.props.sys === "sys"){
                    ...
                    <div className="col-6">                        
                        <ReactHTMLTableToExcel
                            ...
                            className="btn-normal pull-right download-table-xls-button"
                            table="table-to-xls"
                ...
                <div className="row">
                    <div className="col-12"> 
                            ...
                            <table style={{border:'1px solid silver',width:'100%', textAlign:'right'}} 
                                    className="fixed-header-table table-responsive customers table-hover" id="table-to-xls">
                                <thead>
                                <tr>
                                {/* <td colSpan="10" rowSpan="6"></td> */}
                                        ...
                                        <td >
                                            ...
                                            <span id="payTodayBalance" className='btn btn-warning pull-right' style={{color:'#FFFFFF'}}
                                            onClick={()=>{
                                                window.location.href=`/customer/payments/${this.state.item.OrderId}/${this.state.item.OrderAmount}`;
                                            }}
                                            >
                                            {/* <a href={`/customer/payments/${this.state.item.OrderId}/${this.state.item.OrderAmount}`}>Pay</a> */}
                                            </span>
                                        </td>
                                    </tr>
                                    <tr>
    ...
    createTableWithFee = (numberOfDays) => {
        ...
        if(document.getElementById("todayBalance")){
            document.getElementById("todayBalance").innerText = `${currencyFormat(this.state.endingBalance)}`;
            // document.getElementById("todayBalance").innerText)
        }
        if(document.getElementById("payTodayBalance")){
            document.getElementById("payTodayBalance").innerHTML = `<a href='/customer/payments/${this.state.item.OrderId}/${this.state.endingBalance}' className='pull-right'>Make payment</a>`;
        }
